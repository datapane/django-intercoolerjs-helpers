...
from intercooler_helpers.middleware import IntercoolerRedirector, IntercoolerData


@pytest.mark.parametrize(
    "response", [redirect("/test/", permanent=False), redirect("/", permanent=True),]
)
def test_redirects_turn_into_clientside_redirects(rf, response):
    request = rf.get("/", HTTP_X_IC_REQUEST="true", HTTP_X_REQUESTED_WITH="XMLHttpRequest")
...
@pytest.mark.parametrize(
    "response", [HttpResponse(status=201), HttpResponse(status=307), HttpResponse(status=501),]
)
